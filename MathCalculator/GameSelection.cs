...
public class GameSelection
{
	...
	public void GameRequestSelectionUser()
	{
		...
		var table = new Table()
			.Centered()
			.Caption("Game selection options for the user to play.")
			.BorderColor(Color.Green3_1)
			.HeavyBorder()
			.Title("Games Being [purple]Played[/]");
		...
		var grid = new Grid().Centered().Expand();
		...
		for (int i = 0; i < 10; i++)
		{
			...
			// add rows with items per column to grid
			grid.AddRow(MethodNames.Skip(numColumns * i).Take(numColumns).ToArray()).Centered();
		}
		...
		layout["Left"].Update(
			new Panel(
				Align.Center(new Columns(MethodNames.Skip(0).Take(12)))
			)
			.BorderStyle(new Style(Color.Green, Color.Purple, Decoration.Bold))
			.Header("Game Options")
		);
		// update the right top column
		layout["Right"]["Top"].Update(
			new Panel(
				Align.Center(new Columns(MethodNames.Skip(12).Take(24)))
			)
			.BorderStyle(new Style(Color.CadetBlue, Color.LightCoral, Decoration.Bold))
			.Header("Game Options")
		);
		// update the right bottom column
		layout["Right"]["Bottom"].Update(
			new Panel(
				Align.Center(new Columns(MethodNames.Skip(24).Take(totalItems)))
			)
			.BorderStyle(new Style(Color.RosyBrown, Color.BlueViolet, Decoration.Bold))
			.Header("Game Options")
		);
		...
		// took different approach to getting this value or symbol 
		...
		try
		{
			...
			// managed in the ContinueGameSelectOrEnd method
	...
	public void ContinueGameSelectOrEnd(string gameName)
	{
		...
		switch ((double)GameIntro.Score / (double)gameTotal)
		{
			case double i when i == 1.0:
				gameFinalFeedback = $"\nGame over, your Great!!! You are super-awesome. Your final score is {GameIntro.Score}.\n";
				break;
			case double i when i >= 0.8:
				gameFinalFeedback = $"\nGame over, your Great!!! You did well. Your final score is {GameIntro.Score}.\n";
				break;
			case double i when i >= 0.5:
				gameFinalFeedback = $"\nGame over, your Great!!! You did alright. Your final score is {GameIntro.Score}.\n";
				break;
			case double i when i > 0.0:
				gameFinalFeedback = $"\nGame over, your Great!!! You did it, keep trying. Your final score is {GameIntro.Score}.\n";
				break;
			default:
				gameFinalFeedback = "\nKeep trying, you get it!\n";
